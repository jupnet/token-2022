...
const fn init_basis_point() -> U256 {
    U256::new(MAX_FEE_BASIS_POINTS as u128)
}
const ONE_IN_BASIS_POINTS: U256 = init_basis_point();
...
impl TransferFee {
    ...
    ///
    /// `calculate_fee(x) >= calculate_inverse_fee(x - calculate_fee(x))`
    pub fn calculate_inverse_fee(&self, post_fee_amount: U256) -> Option<U256> {
...
impl TransferFeeConfig {
    ...
    pub fn calculate_epoch_fee(&self, epoch: u64, pre_fee_amount: U256) -> Option<U256> {
        self.get_epoch_fee(epoch).calculate_fee(pre_fee_amount)
    }
    /// Calculate the fee for the given epoch and output amount
    pub fn calculate_inverse_epoch_fee(&self, epoch: u64, post_fee_amount: U256) -> Option<U256> {
...
pub(crate) mod test {
    use {
        ...
        spl_pod::primitives::{PodU256, PodU64},
        std::convert::TryFrom,
    };
    ...
    fn calculate_fee_min() {
        ...
        assert_eq!(
            minimum_fee,
            transfer_fee.calculate_fee(U256::new(2)).unwrap()
        );
    ...
    fn calculate_fee_zero() {
        ...
        assert_eq!(0, transfer_fee.calculate_fee(U256::MAX).unwrap());
    ...
    fn calculate_fee_exact_out_max() {
        ...
        assert_eq!(
            maximum_fee,
            transfer_fee
                .calculate_inverse_fee(U256::MAX - maximum_fee)
                .unwrap()
        );
        // at exactly the max
        assert_eq!(
    ...
    fn calculate_pre_fee_amount_edge_cases() {
        ...
        assert_eq!(1, transfer_fee.calculate_pre_fee_amount(U256::ONE).unwrap());
    }
    ...
    fn calculate_fee_exact_out_min() {
        ...
        assert_eq!(
            minimum_fee,
            transfer_fee.calculate_inverse_fee(U256::ONE).unwrap()
        );
        // still minimum at 2 tokens
        assert_eq!(
            minimum_fee,
            transfer_fee.calculate_inverse_fee(U256::new(2)).unwrap()
        );
        ...
        // zero is zero token
        assert_eq!(0, transfer_fee.calculate_inverse_fee(U256::ZERO).unwrap());
    }
